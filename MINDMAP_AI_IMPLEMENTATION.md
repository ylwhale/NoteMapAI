...
1. Open `MindMapAI.xcodeproj` in Xcode.
...
- An AI request contains only the question, displayed retrieval assumption, and relevant excerpts already shown in Ask. It does not contain the full library. Precise coordinates are removed from saved location fields; coordinates deliberately typed into visible note text remain visible and are disclosed before consent.
...
- 56 unit/service/privacy tests pass, including synthesized quiz-plan guidance, adversarial invented-detail rejection, and in-flight consent revocation between AI batches.
